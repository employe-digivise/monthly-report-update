#!/usr/bin/env python3
"""
Draft render: generates HTML preview from template_draft.html + sample_data.json
using styles_draft.css (purple theme redesign).

Usage: python3 execution/test_render_draft.py
"""
import json
import re
import os
...
def main():
    ...
    # Read DRAFT CSS + embed fonts for offline rendering
    with open(TEMPLATES_DIR / "styles_draft.css", "r") as f:
    ...
    # Use DRAFT template
    template = env.get_template("template_draft.html")
    html = template.render(**data)

    # Output
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    html_path = OUTPUT_DIR / "draft_preview.html"
    with open(html_path, "w") as f:
        f.write(html)

    print(f"Draft HTML preview saved: {html_path}")
